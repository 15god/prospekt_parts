...
<template>
    <tr class="border-b dark:border-gray-700">
        ...
        <td class="px-4 py-3">{{ product.article }}</td>
        <td class="px-4 py-3">{{ product.unit }}</td>
        <td class="px-4 py-3">{{ product.price }}₽</td>
        <td class="px-4 py-3">{{ product.quantity }}</td>
        <td class="px-4 py-3 flex items-center justify-end">
            <button v-bind:id="product.name + product.article + '-dropdown-button'" v-bind:data-dropdown-toggle="product.name + product.article + '-dropdown'" class="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100"  type="button">
                <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                </svg>
            </button>
            <div v-bind:id="product.name + product.article + '-dropdown'" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
